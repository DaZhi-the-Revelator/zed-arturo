...
function isInComment(text, lineIndex, charIndex) {
  const lines = text.split("\n");
...
async function validateTextDocument(document) {
  ...
  lines.forEach((line, lineIndex) => {
    ...
    words.forEach((word) => {
      if (BUILTIN_NAMES.has(word) || symbols.variables.has(word) || symbols.functions.has(word) || ["do", "if", "loop", "while", "until", "when", "unless"].includes(word)) {
        return;
      }
      const wordIndex = line.indexOf(word);
      if (line[wordIndex + word.length] === ":") {
        return;
      }
      if (isInComment(text, lineIndex, wordIndex)) {
        return;
      }
      const context = line.substring(Math.max(0, wordIndex - 5), wordIndex + word.length + 5);
...
connection.onDefinition((params) => {
  ...
  if (isInComment(text, position.line, position.character)) {
    return null;
  }
  const wordMatch = line.substring(0, position.character).match(/(\w+)$/);
  if (!wordMatch)
    return null;
...
\`\`\``
  ...
  if (typeMatch && typeMatch[1] === word) {
    if (ARTURO_TYPES[word]) {
      const typeInfo = ARTURO_TYPES[word];
      return {
        contents: {
          kind: MarkupKind.Markdown,
          value: `**:${word}** (type)
