...
var ATTRIBUTE_NAMES = /* @__PURE__ */ new Set([
...
var ARTURO_COLORS = /* @__PURE__ */ new Set([
  "red",
  "green",
  "blue",
  "yellow",
...
var ARTURO_UNITS = /* @__PURE__ */ new Set([
  // Length
  "m",
  "mm",
  "cm",
...
var documentSymbols = /* @__PURE__ */ new Map();
    ...
    if (commentIndex !== -1) {
      ...
      for (let i = 0; i < commentIndex; i++) {
        ...
        if ((char === '"' || char === "{") && !inString) {
          ...
          if (stringChar === '"' && char === '"' && line[i - 1] !== "\\" || stringChar === "{" && char === "}") {
            inString = false;
            stringChar = null;
          }
        }
...
function parseDocument(document) {
  ...
  customTypes.set(document.uri, /* @__PURE__ */ new Set());
...
function inferType(value) {
  ...
  if (value === "null")
    return ":null";
  if (value.startsWith('"') || value.startsWith("{") || value.startsWith("\xAB"))
    return ":string";
  if (/^`.$/.test(value))
    return ":char";
  if (value.startsWith("["))
...
function isLiteral(word) {
  ...
  if (/^0b[01]+$/.test(word))
    return true;
  if (/^(true|false|maybe|null)$/.test(word))
    return true;
  if (/^\d+(-[a-zA-Z][a-zA-Z0-9]*)?$/.test(word))
    return true;
  if (/^\d+(\.\d+)*(-[a-zA-Z][a-zA-Z0-9]*)?$/.test(word))
    return true;
  if (/^[0-9a-fA-F]{6}$/.test(word))
    return true;
  if (/^[0-9a-fA-F]{3}$/.test(word))
    return true;
  if (ARTURO_UNITS.has(word))
    return true;
  if (ARTURO_COLORS.has(word))
    return true;
  if (/^[a-z]$/.test(word))
    return true;
  return false;
    ...
    for (let i = 0; i < endPos; i++) {
      ...
      if (!inString) {
        ...
        if (stringType === "code_block" && prevChar === ":" && char === "}") {
          ...
          inString = false;
          stringType = null;
        } else if (stringType === "safe" && prevChar === "\xBB" && char === "\xBB") {
          inString = false;
          stringType = null;
        }
      }
...
async function validateTextDocument(document) {
  ...
  const allFunctionParams = /* @__PURE__ */ new Set();
  const allLoopVars = /* @__PURE__ */ new Set();
  const allDictKeys = /* @__PURE__ */ new Set();
  ...
  const docTypes = customTypes.get(document.uri) || /* @__PURE__ */ new Set();
  lines.forEach((line, lineIndex) => {
    ...
    words.forEach((word) => {
      ...
      if (BUILTIN_NAMES.has(word) || symbols.variables.has(word) || symbols.functions.has(word)) {
        return;
      }
  ...
  cleanLines.forEach((line, lineIndex) => {
    ...
    if (binaryOpMatch) {
      ...
      const numericTypes = /* @__PURE__ */ new Set([":integer", ":floating", ":complex", ":rational"]);
...
connection.onReferences((params) => {
  ...
  if (BUILTIN_NAMES.has(targetWord))
    return [];
  const references = [];
...
connection.onFoldingRanges((params) => {
  ...
  lines.forEach((line, lineIndex) => {
    ...
    for (let i = 0; i < line.length; i++) {
      const char = line[i];
      if (isInComment(line, i) || isInString(line, i))
        continue;
...
connection.onRequest("textDocument/inlayHint", (params) => {
  ...
  const lines = text.split("\n");
  const hints = [];
  const symbols = documentSymbols.get(params.textDocument.uri) || { variables: /* @__PURE__ */ new Map(), functions: /* @__PURE__ */ new Map() };
  lines.forEach((line, lineIndex) => {
    if (line.trim().startsWith(";"))
      return;
    ...
    while ((match = funcCallRegex.exec(line)) !== null) {
      ...
      const argsStartCol = match.index + match[1].length + 1;
      const funcInfo = BUILTIN_FUNCTIONS[funcName];
      if (!funcInfo || !funcInfo.params)
        continue;
...
function getWordAtPosition(line, character) {
  ...
  if (!beforeMatch && afterMatch && afterMatch[0].length > 0) {
    const word2 = afterMatch[0];
    return { word: word2, start: character, end: character + word2.length };
  }
  if (!beforeMatch)
    return null;
  ...
  for (let i = 0; i < position; i++) {
    ...
    if (!inString) {
      ...
      if (stringChar === '"' && char === '"' && prevChar !== "\\") {
        ...
        inString = false;
        stringChar = null;
      } else if (stringChar === "\xAB" && char === "\xBB") {
        inString = false;
        stringChar = null;
      }
    }
